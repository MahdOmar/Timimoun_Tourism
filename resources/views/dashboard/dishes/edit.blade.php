...
<div class="max-w-7xl mx-auto py-10 px-4">
  ...
  <form action="{{ route('traditionaldish.update',$traditionalDish->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-lg shadow">
    @csrf
    @method('PUT')

   <input type="text" name="id" value="{{ $traditionalDish->id }}" hidden>
 <div class="grid md:grid-cols-3 gap-2">
    ...
    @foreach(['ar' => 'Arabic', 'en' => 'English', 'fr' => 'French'] as $locale => $label)
      <div>
        ...
        <input type="text" name="name[{{ $locale }}]" id="name_{{ $locale }}" value="{{ $traditionalDish->getTranslation('name', $locale) }}"
    ...
    @foreach(['ar' => 'Arabic', 'en' => 'English', 'fr' => 'French'] as $locale => $label)
      <div>
        ...
        <textarea name="description[{{ $locale }}]" id="description_{{ $locale }}" rows="3"
                  ...
                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ $traditionalDish->getTranslation('description', $locale) }}</textarea>
      </div>
    @endforeach
    <!-- Category -->
    ...
    <div>
      <label for="main_image" class="block text-sm font-medium text-gray-700">Main Image</label>
        @if($traditionalDish->main_image)
    <div id="main-image-wrapper" class="relative w-max">
        <img src="{{ asset('storage/' . $traditionalDish->main_image) }}" alt="Main Image" class="w-40 h-32 object-cover mb-2 rounded">

        <button
            type="button"
            class="absolute top-0 right-0 bg-red-600 text-white rounded-full p-1 text-xs hover:bg-red-700"
            onclick="removeMainImage({{ $traditionalDish->id }})"
        >✕</button>
    </div>
@endif
  ...
  <button id="removeMainImageBtn"
    ...
    <div class="grid grid-cols-8 gap-2 mb-4">
     ...
     @foreach ($traditionalDish->gallery as $image)
    <div id="gallery-image-{{ $image->id }}" class="relative w-max">
        ...
        <button
            type="button"
            class="absolute top-0 right-0 bg-red-600 text-white rounded-full p-1 text-xs hover:bg-red-700"
            onclick="removeGalleryImage({{ $traditionalDish->id }}, {{ $image->id }})"
        >✕</button>
    </div>
@endforeach
...
<script>
    async function removeMainImage(accommodationId) {
        ...
        const response = await fetch(`/traditionaldish/${accommodationId}/remove-main-image`, {
            method: 'DELETE',
            headers: {
    ...
    async function removeGalleryImage(accommodationId, imageId) {
        ...
        const response = await fetch(`/traditionaldish/${accommodationId}/gallery/${imageId}/remove`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json'
